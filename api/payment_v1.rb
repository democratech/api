...
END
			...
			get 'total' do
				pg_connect()
				begin
					search_transaction="SELECT count(*) as nb_adherents, sum(amount) as total FROM donations WHERE created>'2017-09-15' AND candidate_id is null AND recipient='PARTI' and (origin='chèque' OR (origin='payzen' AND status='AUTHORISED'))"
